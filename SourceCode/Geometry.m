...
if nargin == 1
    %nargin = 1时即函数只接受了pt一个参数，没有RG参数
    %Geometry radii RG are not given
    RadiiGiven_flag = 0;
else 
    RadiiGiven_flag = 1; 
end
...
%【删除请求】以下两段判断函数目前没有作用
if isfield(pt,'i') && ~isfield(pt,'input' )
    pt.input = pt.i;
end
if isfield(pt,'d') && ~isfield(pt,'design')
    pt.design = pt.d;
end
...
if isfield(pt.input,'Chord_flag')
...
%获取叶型
if isfield(pt.input,'Thickness')
    Thickness = pt.input.Thickness;
else
    Thickness = 'NACA 65A010';
end

%【删除请求】pt.input中没有'LSGeoCorr'参量
...
%【修正请求】pt.input中没有'Make_GeoText_flag'参量
%'Make_GeoText_flag'用于决定后续是否输出'filename_Geometry.txt'文档
%目前是默认输出
if isfield(pt.input,'Make_GeoText_flag')
    Make_GeoText_flag = pt.input.Make_GeoText_flag;  
else
    Make_GeoText_flag = 1;
end

%【修正请求】pt.input中没有'Make_LaTeX_flag'参量
%'Make_LaTeX_flag'用于决定是否输出'filename_LaTeX.txt'文档
%目前修改为默认输出
%'filename_LaTeX.txt'文档和'filename_Geometry.txt'文档内容类似
%前者是将生成的叶片几何内容整理成了LaTeX格式
if isfield(pt.input,'Make_LaTeX_flag')
    Make_LaTeX_flag = pt.input.Make_LaTeX_flag;  
else
    Make_LaTeX_flag = 1;
end
...
if isfield(pt.input,'Vs')
    Vs = pt.input.Vs;
else
    %由于软件界面目前自带缺省值，上方的判断不可能为假，Geometry.m文件中的缺省值无用
    Vs = 1;
end

if isfield(pt.input,'D')
    R = pt.input.D/2;
elseif isfield(pt.input,'R')
    R = pt.input.R;
else
    R = 1;
end

if isfield(pt.input,'Dhub')
    Rhub = pt.input.Dhub/2;
elseif isfield(pt.input,'Rhub')
    Rhub = pt.input.Rhub;
else
    %默认的转轴半径为大半径的0.2
    Rhub = 0.2*R;
...
%【删除请求】之前的程序中没有采用Rcirc & Rroot这种命名方式的转轴半径
if isfield(pt.input,'Rcirc')
    Rcirc = pt.input.Rcirc;
else
    Rcirc = Rhub;
end
if isfield(pt.input,'Rroot')
    Rroot = pt.input.Rroot;
else
    Rroot = Rhub;
end

if isfield(pt.input,'Np')
    Np = pt.input.Np;
else
    Np = 20;
end
