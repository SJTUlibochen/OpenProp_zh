...
% 
% 这个函数根据给定的弦径比分布拟合样条曲线，以对指定半径处的弦径比进行插值
% 相较于matlab中给定的插值方法，该函数在有必要时可以保持叶梢处的尖锐
%
% INPUTS
...
function CoD = InterpolateChord(XR,XCoD,RG)
    if (abs(XR(end)-1) < 1e-4) && (XCoD(end) > 0.01)  
        % if XR == 1 and XCoD > 0.01 at tip  (i.e. finite chord at the tip)
        CoD = pchip(XR,XCoD,RG);   % allow for finite chord at tip
    else
        % Assume near-zero chord at tip, and interpolate using
        % a B-spline curve fit to the expanded blade outline
        [~,N] = size(RG);
        XR   =   XR(:)';
        XCoD = XCoD(:)';
        RG   =   RG(:);
